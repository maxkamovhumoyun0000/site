...
function DiamondVoyQuizModal({
  quiz,
  onClose,
  onReviewWithDiamondvoy,
}: {
  quiz: QuizPayload;
  onClose: () => void;
  onReviewWithDiamondvoy: (context: TestReviewContext) => void;
}) {
  const rawQuestions = Array.isArray(quiz.questions) ? quiz.questions : [];
  const questions = useMemo(() => {
    return rawQuestions.map((q) => {
      const opts = Array.isArray(q.options)
        ? Array.from(new Set(q.options.map((o) => String(o).trim()))).filter(Boolean).slice(0, 4)
        : [];
      return {
        prompt: String(q.prompt || "Savol"),
        options: opts,
        correct_answer: String(q.correct_answer || opts[0] || ""),
  ...
  const isCurrentSubmitted = Boolean(submittedAnswers[currentIndex]);
  ...
  const handleCheckCurrent = () => {
    if (!currentSelected || isCurrentSubmitted) return;
  ...
  const correctCount = useMemo(() => {
    return questions.reduce((acc, q, idx) => {
      ...
      return acc + (chosen && chosen.trim().toLowerCase() === q.correct_answer.trim().toLowerCase() ? 1 : 0);
    }, 0);
  }, [questions, selectedAnswers]);

  const reviewItems: TestReviewItem[] = useMemo(() => {
    return questions.map((q, idx) => {
      const chosen = selectedAnswers[idx];
      const isCorrect = Boolean(chosen && chosen.trim().toLowerCase() === q.correct_answer.trim().toLowerCase());
      return {
        prompt: q.prompt,
        options: q.options,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-6 animate-fade-in">
      <div className="relative flex h-full max-h-[94vh] w-full max-w-xl flex-col overflow-hidden rounded-3xl border-2 border-slate-200 bg-white shadow-2xl dark:border-navy-700 dark:bg-[#131f24]">
        {/* Top bar */}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-7">
          {!isFinished && currentQ ? (
            <div>
              <span className="inline-block rounded-lg bg-cyan-500/10 px-2.5 py-1 text-xs font-black text-cyan-700 dark:text-cyan-300">
                Savol {currentIndex + 1}
              </span>
              <p className="mt-3 text-lg font-bold leading-relaxed text-navy-900 dark:text-white">
                {currentQ.prompt}
              </p>
              ...
              <div className="mt-6 space-y-3">
                {currentQ.options.map((opt, optIdx) => {
                  ...
                  let optionClass = "w-full rounded-2xl border-2 p-4 text-left font-bold text-sm transition-all ";
                  if (!isCurrentSubmitted) {
                    optionClass += isSelected
                      ? "border-cyan-500 bg-cyan-50 text-cyan-800 ring-2 ring-cyan-200 dark:bg-cyan-950/40 dark:text-cyan-200 dark:ring-cyan-800"
                      : "border-slate-200 bg-white text-navy-900 hover:border-cyan-300 dark:border-white/10 dark:bg-navy-900/50 dark:text-white";
                  } else {
                    if (isCorrectOpt) {
                      optionClass += "border-emerald-500 bg-emerald-50 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200";
                    } else if (isSelected && !isCorrectOpt) {
                      optionClass += "border-rose-500 bg-rose-50 text-rose-800 dark:bg-rose-950/40 dark:text-rose-200";
                    } else {
                      optionClass += "border-slate-200 bg-slate-50/50 text-slate-400 opacity-60 dark:border-white/5 dark:bg-navy-950/50";
                    }
                  }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-black/5 text-xs font-black dark:bg-white/10">
                          {String.fromCharCode(65 + optIdx)}
                        </span>
                        <span className="flex-1">{opt}</span>
                        {isCurrentSubmitted && isCorrectOpt && <span className="text-emerald-600 font-bold">✓</span>}
                        {isCurrentSubmitted && isSelected && !isCorrectOpt && <span className="text-rose-600 font-bold">✗</span>}
                      </div>
                    </button>
                  );
                })}
              ...
              {isCurrentSubmitted && (
                <div className="mt-5 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4 dark:bg-cyan-950/30 animate-fade-in">
                  <p className="text-xs font-black text-cyan-800 dark:text-cyan-200">
                    💡 Tushuntirish & Qoida:
                  </p>
                  <p className="mt-1 text-sm leading-relaxed text-ink-700 dark:text-navy-200">
                    {currentQ.explanation || `To‘g‘ri javob: ${currentQ.correct_answer}`}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="py-6 text-center animate-fade-in">
              <div className="relative mx-auto mb-4 grid h-24 w-24 place-items-center rounded-full border-4 border-cyan-500 bg-gradient-to-b from-cyan-400 to-blue-600 text-5xl shadow-2xl text-white">
                {correctCount >= 7 ? "🏆" : "💪"}
              </div>
              ...
              <p className="mt-1 text-sm font-semibold text-ink-500 dark:text-navy-300">
                {quiz.title}
              </p>

              {/* Stats */}
              <div className="mt-6 grid grid-cols-3 gap-3">
                <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-3 dark:border-navy-700 dark:bg-navy-900/60">
                ...
                <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-3 dark:border-emerald-800 dark:bg-emerald-950/40">
                ...
                <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/40">
                  <span className="text-xs font-black uppercase text-amber-700 dark:text-amber-300">Ball</span>
              ...
              {/* Test Completion Actions - with Diamondvoy analysis */}
              <div className="mt-6">
              ...
              <button
                ...
                className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-500 py-3.5 text-sm font-black text-white shadow-lg shadow-cyan-500/20 transition hover:bg-cyan-600 dark:text-navy-950"
              >
                <span>💎</span>
                <span>Ushbu natijani Diamondvoy bilan tahlil qilish</span>
              </button>

              <button
                type="button"
                onClick={onClose}
                className="mt-3 w-full rounded-2xl border border-slate-200 bg-white py-3 text-sm font-bold text-slate-600 transition hover:bg-slate-50 dark:border-navy-700 dark:bg-navy-800 dark:text-navy-300"
              >
                Yopish
              </button>
            </div>
        ...
        {/* Bottom controls */}
        {!isFinished && (
          <div className="flex items-center justify-between border-t border-slate-100 p-4 dark:border-white/10 bg-slate-50 dark:bg-navy-900/60">
            <button
              type="button"
              onClick={handlePrev}
              disabled={currentIndex === 0}
              className="rounded-xl border border-line px-4 py-2.5 text-xs font-black text-ink-600 disabled:opacity-40 dark:border-white/10 dark:text-white"
            >
              ← Oldingi
            </button>
            ...
            {!isCurrentSubmitted ? (
              <button
                ...
                className="rounded-xl bg-cyan-500 px-6 py-2.5 text-xs font-black text-white shadow-md shadow-cyan-500/20 disabled:opacity-40 hover:bg-cyan-600 dark:text-navy-950"
              >
                Tekshirish
              </button>
            ) : (
              <button
                type="button"
                onClick={handleNext}
                className="rounded-xl bg-[#002DFF] px-6 py-2.5 text-xs font-black text-white shadow-md shadow-blue-500/20 hover:bg-blue-700"
              >
...
function DiamondVoyQuizCard({
  ...
  const qCount = Array.isArray(quiz.questions) ? quiz.questions.length : 10;

  return (
    <>
      <div className="rounded-2xl border-2 border-cyan-500/30 bg-gradient-to-br from-cyan-50/90 via-sky-50/50 to-indigo-50/60 p-4 shadow-sm dark:border-cyan-500/20 dark:from-cyan-950/30 dark:via-navy-900/40 dark:to-indigo-950/30">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-cyan-500 text-xl text-white shadow-md shadow-cyan-500/25">
              🎯
            </span>
            <div>
              <span className="inline-block rounded-md bg-cyan-500/15 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-cyan-700 dark:text-cyan-300">
                Diamondvoy Testi
              </span>
              <h4 className="text-base font-black text-navy-900 dark:text-white">
                {quiz.title || "10 ta savolli test"}
              </h4>
            </div>
          </div>
          <span className="shrink-0 rounded-full bg-cyan-100 px-2.5 py-1 text-xs font-black text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-200">
            {qCount} ta savol
          </span>
        </div>

        {quiz.topic ? (
          <p className="mt-2 text-xs font-semibold text-ink-600 dark:text-navy-200">
            Mavzu: <span className="text-cyan-700 dark:text-cyan-300">{quiz.topic}</span>
          </p>
        ) : null}

        <div className="mt-3 flex items-center gap-2 text-xs text-ink-500 dark:text-navy-300">
          <span>✨ 4 ta noyob variant</span>
          <span>•</span>
          <span>💡 Qoidalar tahlili</span>
          <span>•</span>
          <span>💎 Diamondvoy tekshiruvi</span>
        </div>

        <button
          type="button"
          onClick={() => setModalOpen(true)}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 py-3 text-sm font-black text-white shadow-lg shadow-cyan-500/25 transition hover:from-cyan-600 hover:to-blue-700 active:scale-[0.98]"
        >
          <span>🚀</span>
          <span>Testni boshlash ({qCount} ta savol)</span>
        </button>
      </div>
...
export function UniversalChat({
  ...
  // Initial prompt hand-off (e.g. from personal plan "Diamondvoy bilan o'rganish")
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const prompt = window.sessionStorage.getItem("diamondvoy:initial-prompt:v1");
      if (!prompt) return;
      window.sessionStorage.removeItem("diamondvoy:initial-prompt:v1");
      setActivePane("diamondvoy");
      window.setTimeout(() => sendDiamondvoyMessage(prompt).catch(() => null), 0);
    } catch {
      window.sessionStorage.removeItem("diamondvoy:initial-prompt:v1");
      ...
      >
        {!activeChatId ? (
            ...
            {isStudent && (
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const topic = window.prompt("Qaysi mavzu bo‘yicha 10 ta test tuzib beray? (masalan: Present Simple, Passive Voice)") || "Ingliz tili";
                    if (topic.trim()) {
          ...
          aiMessages.map((message) => {
            ...
            const isWizardTrigger = typeof message.content === 'string' && (message.content.includes('"wizard_trigger"') || message.content.includes('"quiz_runner"') || message.content.includes('quiz_json'));
            return (
                ...
                >
                  {message.streaming && !message.content ? (
                    ...
                    try {
                      ...
                      if (parsed.wizard === "quiz_runner" || (parsed.type === "wizard_trigger" && parsed.wizard === "quiz_runner")) {
                        return <DiamondVoyQuizCard quiz={parsed} onReviewWithDiamondvoy={(ctx) => {
                          setTestReviewContext(ctx);
                          sendDiamondvoyMessage(testReviewIntroduction(ctx)).catch(() => null);
                        }} />;
                      }
                    } catch {}

                    if (typeof message.content === "string") {
                      const quizMatch = message.content.match(/```(?:quiz_json|json)?\s*([\s\S]*?"wizard"\s*:\s*"quiz_runner"[\s\S]*?)\s*```/);
                      if (quizMatch && quizMatch[1]) {
                        try {
                          const parsedQuiz = JSON.parse(quizMatch[1]);
                          if (parsedQuiz?.wizard === "quiz_runner") {
                            const textBefore = message.content.replace(quizMatch[0], "").trim();
                            return (
                              <div className="space-y-3">
                                {textBefore ? renderChatText(textBefore) : null}
                                <DiamondVoyQuizCard quiz={parsedQuiz} onReviewWithDiamondvoy={(ctx) => {
                                  setTestReviewContext(ctx);
                                  sendDiamondvoyMessage(testReviewIntroduction(ctx)).catch(() => null);
                                }} />
                              </div>
