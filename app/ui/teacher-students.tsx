...
export function TeacherStudents({ token, onApiCall }: { token: string; onApiCall: any }) {
  ...
  const filteredStudents = students.filter((s: any) => {
    ...
    return fullName.includes(q) || phone.includes(q) || subject.includes(q);
  });

  return (
    <div className="page-stack">
      <div className="row-between mb-4 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <h2>{tt("students", "Mening O'quvchilarim")} ({filteredStudents.length}{searchQuery ? ` / ${students.length}` : ""})</h2>
          {loading && <span className="chip">Yuklanmoqda...</span>}
        </div>
        <div className="relative">
          <input
            type="text"
            className="w-64 rounded-xl border border-line bg-surface-soft px-3 py-1.5 text-xs font-semibold text-navy-900 outline-none transition focus:border-cyan-500 dark:border-white/10 dark:bg-white/5 dark:text-white"
          ...
          {searchQuery ? (
            <button
              type="button"
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-ink-400 hover:text-ink-600 dark:text-navy-400"
