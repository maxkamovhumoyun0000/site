...
export function StudentVoiceRoom({ data, userSubjects = [] }: { data: Record<string, any>, userSubjects?: string[] }) {
  const t = useWebT();
  ...
  useEffect(() => {
    if (roomIdParam && state === "lobby" && !errorMsg) {
  ...
  // Auto-record logic
  useEffect(() => {
    if (hwId && state === "room" && roomState?.stagePeers && !hasSubmitted) {
      if (!isRecording && roomState.stagePeers.length >= 2) {
        startRecording();
      }
    }
  }, [hwId, state, roomState?.stagePeers, isRecording, startRecording, hasSubmitted]);

  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hwId && isRecording && !hasSubmitted) {
        e.preventDefault();
        e.returnValue = "Hali audio topshirilmadi. Chiqib ketsangiz yozuv yo'qoladi, aniqmi?";
        return e.returnValue;
      }
    };
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => window.removeEventListener("beforeunload", handleBeforeUnload);
  }, [hwId, isRecording, hasSubmitted]);

  const handleLeaveRoom = async () => {
    if (hwId && isRecording && !hasSubmitted) {
      setIsSubmittingHw(true);
      await stopRecordingAndUpload(hwId);
  ...
  const handleCreateRoom = async () => {
    await createRoom(createName || undefined, createSubject || undefined);
  ...
  if (state === "lobby") {
    ...
    return (
      <div className="w-full h-[100dvh] flex flex-col bg-slate-50 dark:bg-[#0a0f1c] relative overflow-hidden">
        ...
        {isSubmittingHw && (
          <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-950/80 backdrop-blur-sm">
            ...
            <h2 className="text-xl font-bold text-white">Vazifa topshirilmoqda...</h2>
            <p className="text-slate-400 mt-2">Iltimos kuting, audio yuklanmoqda.</p>
          </div>
        )}
        ...
        <div className="pt-6 pb-4 px-6 bg-white dark:bg-slate-900 shadow-sm border-b border-slate-200 dark:border-slate-800 flex items-center justify-between z-20">
           ...
           <button
             ...
             className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all flex items-center gap-1.5"
           >
             <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
             <span>{t("voiceroom.createRoom") || "Xona yaratish"}</span>
           </button>
        </div>
        ...
        {errorMsg && (
          <div className="z-20 m-4 bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 p-3 rounded-xl text-center shadow-sm text-sm border border-red-200 dark:border-red-800/50">
            {errorMsg}
            <button onClick={() => setErrorMsg("")} className="ml-3 underline">{t("voiceroom.close") || "Yopish"}</button>
          </div>
        )}
            ...
            {subjects.length > 2 && (
              <div className="flex space-x-2 overflow-x-auto pb-2 custom-scrollbar">
                {subjects.map(subj => (
                  ...
                  >
                    {subj === "all" ? (t("voiceroom.all_subjects") || "Barcha fanlar") : SUBJECT_MAP[subj] || subj}
                  </button>
                ))}
              </div>
            )}
        ...
        <button 
          ...
          className="fixed bottom-6 right-6 w-14 h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-[0_8px_30px_rgb(79,70,229,0.3)] flex items-center justify-center transition-transform hover:scale-105 active:scale-95 z-30"
          title={t("voiceroom.createRoom") || "Xona yaratish"}
        >
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 animate-in slide-in-from-bottom-4">
              <div className="p-6">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">{t("voiceroom.tags") || "Teglar (Tags)"}</label>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950 overflow-hidden">
      ...
      <div className="z-20 pt-6 pb-4 px-6 flex items-center justify-between bg-gradient-to-b from-slate-950 to-transparent">
        ...
        <div className="flex items-center">
          {/* Homework Recording Controls */}
          {hwId ? (
            isRecording ? (
              <div className="relative w-auto px-3 h-8 mr-2 rounded-full bg-red-600/20 text-red-500 border border-red-500/30 flex items-center justify-center font-bold text-[10px] sm:text-xs gap-2">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                Yozib olinmoqda...
              </div>
            ) : hasSubmitted ? (
              <div className="relative w-auto px-3 h-8 mr-2 rounded-full bg-green-600/20 text-green-500 border border-green-500/30 flex items-center justify-center font-bold text-[10px] sm:text-xs gap-2">
                ✓ Topshirildi
              </div>
            ) : (
              <div className="relative w-auto px-3 h-8 mr-2 rounded-full bg-slate-800 text-slate-400 border border-slate-700 flex items-center justify-center font-bold text-[10px] sm:text-xs gap-2">
                Sherik kutilmoqda...
              </div>
            )
          ) : null}
          ...
          {isHost ? (
            ...
            <button onClick={endRoom} className="relative w-10 h-10 mr-2 rounded-full bg-red-600/80 text-white hover:bg-red-500 flex items-center justify-center backdrop-blur-md border border-red-500/50 transition-colors shadow-lg shadow-red-500/30" title={t("voiceroom.force_close") || "Majburiy yopish"}>
      ...
      <div className="z-10 flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* STAGE AREA */}
        <div className="flex-none lg:flex-1 max-h-[35vh] lg:max-h-none overflow-y-auto px-1 lg:px-8 py-4 custom-scrollbar">

          <div className="grid grid-cols-4 gap-2 max-w-2xl mx-auto px-1 sm:px-4">
            {stagePeers.map(peerId => {
              ...
              className="flex flex-col items-center justify-center relative cursor-pointer group"
                >
                  <div className={cx(
                    "w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-slate-800 mb-2 flex items-center justify-center border-4 z-10 overflow-hidden relative transition-all duration-300 group-hover:scale-105",
                  ...
                  <div className="font-bold text-[10px] sm:text-xs text-white truncate max-w-[95%] text-center">
            ...
            {Array.from({ length: emptyStageSpots }).map((_, i) => (
              <div key={i} className="flex flex-col items-center justify-center opacity-50">
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-slate-800/30 mb-2 flex items-center justify-center border-2 border-dashed border-slate-700/50">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                </div>
                <span className="text-slate-500 font-medium text-[9px] sm:text-xs">{t("voiceroom.empty_spot") || "Empty"}</span>
              </div>
            ))}
          </div>
        </div>
                ...
                <div key={msg.id} className={cx("animate-in fade-in flex flex-col", isMe ? "items-end" : "items-start")}>
                  {!isSystem && (
                    <div className={cx("flex items-baseline gap-2 mb-1 px-1", msg.is_admin ? "text-red-400" : "text-slate-400")}>
                      ...
                      {msg.is_admin && <span className="bg-red-600 text-white text-[9px] px-1.5 py-0.5 rounded shadow-sm shadow-red-500/20 uppercase font-black tracking-wider border border-red-400/50 animate-pulse">{t("admin.users.role.admin") || "Admin"}</span>}
                      <button onClick={() => setReplyingTo(msg)} className="text-[10px] text-slate-500 hover:text-indigo-400 transition-colors uppercase font-bold tracking-wider">{t("voiceroom.reply") || "Reply"}</button>
                    </div>
                  )}
                  {isSystem ? (
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-1.5 w-full">
              <form onSubmit={handleSendChat} className="flex-1 flex gap-1.5">
                <input
                  ...
                  placeholder={t("voiceroom.chat_placeholder") || "Xabar..."}
              ...
              <div className="flex items-center gap-1 shrink-0 ml-auto">
                ...
                {isOnStage ? (
                  <>
                    ...
                    {(!isHost && !isHiddenSpeaker) && (
                      <button onClick={leaveStage} className="w-7 h-7 shrink-0 rounded-full bg-slate-800 text-slate-300 hover:bg-red-500/20 hover:text-red-500 flex items-center justify-center transition-all" title={t("voiceroom.leaveStage") || "Sahnadan tushish"}>
                ...
                {isAdmin && (
                  <>
                    <button onClick={toggleHiddenSpeaker} className={cx("px-3 py-1 text-white rounded-lg flex items-center justify-center text-[10px] uppercase font-bold transition-all shadow-sm ml-1", isHiddenSpeaker ? "bg-purple-600 hover:bg-purple-500" : "bg-slate-700 hover:bg-slate-600")}>
                      {isHiddenSpeaker ? (t("voiceroom.ghost_off") || "Arvoh O'chirish") : (t("voiceroom.ghost_on") || "Arvoh yoqish")}
                    </button>
                    {!isOnStage && (
                      <button onClick={forceJoinStage} className="px-3 py-1 bg-red-600 hover:bg-red-500 text-white rounded-lg flex items-center justify-center text-[10px] uppercase font-bold transition-all shadow-sm ml-1">
                        {t("voiceroom.force_stage") || "Sahnaga"}
                      </button>
                    )}
                  </>
                )}
          ...
          <div className="bg-slate-900 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-slate-800 animate-in zoom-in-95">
            <div className="p-6">
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar">
                {raisedHands.length === 0 ? (
                  <p className="text-center text-slate-500 py-4">{t("voiceroom.no_raised_hands") || "Sahnaga chiqmoqchi bo'lganlar yo'q."}</p>
                ) : (
                  raisedHands.map(hand => (
                    <div key={hand.id} className="flex items-center justify-between p-3 bg-slate-800 rounded-xl">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => rejectStage(hand.id)} className="px-3 py-1.5 bg-slate-700 text-slate-300 rounded-lg text-xs font-bold hover:bg-slate-600">{t("voiceroom.reject") || "Rad etish"}</button>
                        <button onClick={() => approveStage(hand.id)} className="px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-xs font-bold hover:bg-emerald-600">{t("voiceroom.approve") || "Ruxsat"}</button>
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="bg-slate-900 w-full sm:w-96 rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl border-t sm:border border-slate-800 animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 sm:zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="w-full space-y-3">
                ...
                {(isHost || isAdmin) && selectedProfile !== myId && selectedProfile !== roomState.hostId && (
