...
export function StudentVoiceRoom({ data, userSubjects = [] }: { data: Record<string, any>, userSubjects?: string[] }) {
  ...
  const formatDuration = (start: string, end?: string) => {
    if (!end) return t("voiceroom.ongoing_time") || "Davom etmoqda...";
  ...
  if (state === "lobby") {
    ...
    return (
      <div className="w-full h-[100dvh] flex flex-col bg-slate-50 dark:bg-[#0a0f1c] relative overflow-hidden">
                    ...
                    return (
                      <div key={room.id} className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 self-end sm:self-auto">
                          ...
                          >
                            {isActive ? t("voiceroom.join") : (t("common.start") || "Start")}
                          </button>
                        </div>
                      </div>
                    );
        ...
        <div className="flex-none w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 pb-safe z-30 fixed bottom-0 left-0 right-0 sm:relative">
